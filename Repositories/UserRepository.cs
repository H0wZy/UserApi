...
{
    public async Task<User?> GetByEmailAsync(string email) => await Context.Users.AsNoTracking().FirstOrDefaultAsync(u => u.Email.Value == email);
    ...
    public async Task<User?> GetByCpfAsync(string cpf) => await Context.Users.AsNoTracking().FirstOrDefaultAsync(u => u.Cpf.Value == cpf);
    public async Task<bool> GetEmailExistenceAsync(string email) => await Context.Users.AnyAsync(u => u.Email.Value == email);
